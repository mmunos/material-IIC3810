...
\title[Los m�todos Markov chain Monte Carlo (MCMC)]
{Los m�todos Markov chain Monte Carlo (MCMC)}
...
\begin{document}
	...
	\begin{frame}
		...
		{\small
			...
			Ya vimos como los problemas de conteo se pueden reducir a problemas de generaci�n uniforme. 
			\begin{itemize}
				\visible<2->{\item �Pero c�mo podemos resolver el problema de generaci�n uniforme? Las cadenas de Markov son una herramienta �til para esto}
			\end{itemize}
			
			\vs{8}
			
			\visible<3->{
				En general, las cadenas de Markov son una herramienta �til para generar elementos de un conjunto $\Omega$ seg�n una distribuci�n de probabilidad $\vec \pi$}
			\begin{itemize}
				\visible<4->{\item Para esto, la distribuci�n estacionaria de la cadena de Markov debe ser $\vec \pi$}
			\end{itemize}
	...
	\begin{frame}
		...
		{\footnotesize
			...
			\begin{ejemplo}
				Sea $\varphi$ una f�rmula proposicional satisfacible, y sea $\{x_1, \ldots, x_n\}$ el conjunto de variables mencionadas en $\varphi$
				
				\vs{6}
				
				Definimos $\Omega$ como el conjunto de valuaciones $\sigma : \{x_1, \ldots, x_n\} \to \{0,1\}$ y $\vec \pi$ como una distribuci�n de probabilidad tal que:
				\begin{itemize}
				...
				Una cadena de Markov con distribuci�n estacionaria $\vec \pi$ puede ser utilizada para generar valuaciones que satisfacen $\varphi$
				\begin{itemize}
					\visible<2->{\item �C�mo se hace esto? �Cu�ntos pasos toma generar una valuaci�n?}
				\end{itemize}
			\end{ejemplo}
	...
	\begin{frame}
		...
		{\footnotesize
			...
			Un m�todo Markov Chain Monte Carlo utiliza una cadena de Markov para generar elementos de acuerdo con la distribuci�n estacionaria de la cadena
			\begin{itemize}
				\item En el cap�tulo anterior estudiamos uno de estos m�todos
			\end{itemize}
			
			\vs{6}
			
			\visible<2->{
				En este cap�tulo vamos a ver respuestas para dos preguntas fundamentales para los m�todos Markov Chain Monte Carlo}
			\vs{1}
			\begin{itemize}
				\visible<3->{\alert{\item �Es posible crear una cadena de Markov con distribuci�n estacionaria $\vec \pi$ a partir de una cadena de Markov que no tenga a $\vec \pi$ como distribuci�n estacionaria?}}
				
				\vs{1}
				
				\visible<4->{\alert{\item �Cu�ntos pasos deben ser ejecutados para generar elementos de acuerdo a una distribuci�n de probabilidad cercana a la distribuci�n estacionaria de una cadena de Markov?}}
				\begin{itemize}
					{\footnotesize \visible<5->{\alert{\item �C�mo se define que dos distribuciones sean cercanas?}}}
				\end{itemize}
			\end{itemize}
	...
	\begin{frame}
		...
		{\footnotesize
			...
			Dado un conjunto $\Omega$ finito y una distribuci�n de probabilidades $\vec \pi$ sobre $\Omega$, queremos definir una cadena de Markov cuyos estados sean los elementos de $\Omega$ y cuya distribuci�n estacionaria sea $\vec \pi$
			\vs{1}
			\begin{itemize}
				\item Queremos definir esta cadena de Markov a partir de una cadena de Markov dada (que no tiene a $\vec \pi$ como distribuci�n estacionaria) 
			\end{itemize}
	...
	\begin{frame}
		...
		{\small
			...
			\begin{proposition}
				Sea $\{ X_t \}_{t \in \mathbb{N}}$ una cadena de Markov con conjunto de estados $\Omega$ y matriz de transici�n $P$. Si para todo $a,b\in\Omega$ se cumple que:
				\begin{eqnarray*}
					P[b,a] \cdot \vec \pi[a]  & = & P[a,b] \cdot \vec \pi[b],
				\end{eqnarray*}
				entonces $\vec \pi$ es una distribuci�n estacionaria para  $\{ X_t \}_{t \in \mathbb{N}}$    
			\end{proposition}
	...
	%--------------------------------------------------
	\begin{frame}
		\frametitle{Una demostraci�n de la proposici�n}
		
		{\footnotesize
			
			Valores propios de una cadena de Markov Dado $a\in\Omega$, tenemos que:
			\begin{eqnarray*}
				(P \vec \pi)[a] 
			...
			\visible<2->{
				Vale decir, $P \vec \pi = \vec \pi$, de lo cual concluimos que $\vec \pi$ es una distribuci�n estacionaria de $\{ X_t \}_{t \in \mathbb{N}}$ \qed
			}
	...
	\begin{frame}
		...
		{\footnotesize
			...
			\begin{eqnarray*}
				P[b,a] \cdot \vec \pi[a] & = & P[a,b] \cdot \vec \pi[b]
			\end{eqnarray*}
			se le llama {\bf condici�n de reversibilidad}
			
			\vs{8}
			
			\visible<2->{
				La proposici�n nos indica que para que la cadena de Markov tenga a $\vec \pi$ como distribuci�n estacionaria es suficiente que cumpla la condici�n de reversibilidad.}
	...
	%--------------------------------------------------
	\begin{frame}
		\frametitle{Reversibilidad no es una condici�n necesaria}
		
		{\footnotesize
			
			\begin{ejercicio}
				Construya una cadena de Markov irreducible, aperi�dica, con matriz de transici�n $P$ no sim�trica y que tenga a la distribuci�n uniforme $\vec \pi$ como distribuci�n estacionaria.
				\begin{itemize}
					\item N�tese que esta cadena no satisface la propiedad de reversibilidad para $\vec \pi$
				\end{itemize}
			\end{ejercicio}
			...
			\visible<2->{
				Una soluci�n para el ejercicio:
				\alert{
					$P = \ \begin{pmatrix}
	...
	\begin{frame}
		...
		{\small
			...
			\visible<2->{
				No existe ninguna garant�a de que para todo $a,b$ se cumpla que:
				\begin{eqnarray*}
					P[b,a] \cdot \vec \pi[a] & = & P[a,b] \cdot \vec \pi[b]
			\end{eqnarray*}}
	...
	\begin{frame}
		...
		{\small
			...
			Una idea natural para lograr que esto sea una igualdad es hacer m�s peque�o el lado izquierdo. 
			\begin{itemize}
				\item El lado derecho no es necesario hacerlo m�s peque�o
			\end{itemize}
	...
	\begin{frame}
		...
		{\footnotesize
			...
			An�logamente, para un par $a,b$ tal que $ P[b,a] \cdot \vec \pi[a] <  P[a,b] \cdot \vec \pi[b]$, 
			definimos $\alpha[b,a] = 1$ y
			\begin{eqnarray*}
				\alpha[a,b] & = & \frac{P[b,a] \cdot \vec \pi[a]}{P[a,b] \cdot \vec \pi[b]},
			\end{eqnarray*}
			que es lo mismo que la ecuaci�n en el otro caso s�lo que con los roles de $a$ y~$b$~intercambiados.
	...
	\begin{frame}
		...
		{\footnotesize
			...
			Por tanto, nos gustar��a definir $Q[a,b]=P[a,b] \cdot \alpha[a,b]$ y as�� tener una matriz de transici�n $Q$ que cumple la condici�n de reversibilidad. 
			
			\vs{8}
			
			\visible<2->{
				Sin embargo, no podemos asegurar que $Q$ sea la matriz de transici�n de una cadena de Markov.}
			\begin{itemize}
			...
			\visible<4->{De hecho s�lo podemos asegurar que ${\displaystyle \sum_{a \in \Omega} Q[a,b] \leq 1}$ para todo $b \in \Omega$
				\begin{itemize}
					\item �Por qu�?
			\end{itemize}}
	...
	\begin{frame}
		...
		{\footnotesize
			...
			\visible<2->{
				De esta forma, la definici�n de $Q$ es:
				\alert{
					\begin{eqnarray*}
						Q[a,b] & = &  
						\begin{cases}
			...
			\visible<3->{
				\begin{ejercicio}
					Demuestre que $Q$ es la matriz de transici�n de una cadena de Markov.
			\end{ejercicio}}
	...
	\begin{frame}
		...
		{\footnotesize
			...
			\visible<2->{
				Adem�s, sea $a_0$ un elemento arbitrario de $\Omega$}
			
			\vs{6}
			
			\visible<3->{
				El siguiente algoritmo simula $N$ transiciones de acuerdo a la cadena de Markov con matriz de transici�n $Q$ descrita anteriormente.
				%\begin{itemize}
				%\item As��, la cadena de Markov simulada tiene como distribuci�n estacionaria a la distribuci�n $\vec \pi$
				%\end{itemize}
			}
	...
	\begin{frame}
		...
		{\footnotesize
			...
			�C�mo interpretamos la salida del algoritmo? 
			\begin{itemize}
				\item Vamos a demostrar que para cada $j\in\{1, \ldots, N\}$, se tiene que $a_j$ es generado seg�n la distribuci�n de probabilidades $Q^j \vec x_0$, donde $\vec x_0$ es un vector tal que $\vec x_0[a_0] = 1$ y $\vec x_0[b] = 0$ para todo $b \neq a_0$
			\end{itemize}
			
			\vs{8}
			
			\visible<2->{
				En este contexto, $Q$ es la matriz que describimos antes, con distribuci�n estacionaria $\vec \pi$ 
				\begin{itemize}
					\item As��, el algoritmo toma un vector inicial $\vec x_0$ y realiza $N$ transiciones seg�n la matriz de transici�n $Q$
			\end{itemize}}
	...
	\begin{frame}
		...
		{\footnotesize
			...
			El algoritmo de Metropolis-Hastings nos entrega una estrategia general para muestrear de acuerdo a una distribuci�n de inter�s $\vec \pi$
			\begin{itemize}
				\item Si $N$ es suficientemente grande, entonces $a_N$ ser� generado de acuerdo a una distribuci�n similar a la estacionaria
			\end{itemize}
			
			\vs{6}
			
			\visible<2->{
				Sin embargo, el algoritmo no entrega a priori ning�n criterio respecto a qu� se entiende por suficientemente grande.
				\begin{itemize}
					\item �Cu�l debe ser el valor de $N$ para que la cadena de Markov se acerque lo suficiente a la distribuci�n estacionaria? 
			\end{itemize}}
	...
	\begin{frame}
		...
		{\small
			...
			\begin{itemize}
				\item Eso significa que $\vec \pi$ es la distribuci�n uniforme, y que queremos estar muy cerca de la distribuci�n estacionaria
	...
	\begin{frame}
		...
		{\footnotesize
			...
			Otro tema importante es el de la convergencia de la cadena de Markov a la distribuci�n $\vec \pi$
			\begin{itemize}
				\item El algoritmo de Metropolis-Hastings nos asegura que $\vec \pi$ es distribuci�n estacionaria, pero no nos asegura que sea la �nica
				
				\item A�n m�s, el algoritmo no nos asegura que la cadena de Markov converja a $\vec \pi$ independientemente del vector $\vec x_0$ inicial
			\end{itemize}
			
			\vs{8}
			
			\visible<2->{
				Para conseguir estas garant��as se debe exigir m�s condiciones a la cadena de Markov caracterizada por $Q$
				\begin{itemize}
					\item M�s adelante vamos a analizar en detalle c�mo conseguir estas garant�as manipulando la matriz $P$
			\end{itemize}}
	...
	\begin{frame}
		...
		{\footnotesize
			...
			C�mo mencionamos antes, en los problemas de conteo consideramos $\vec \pi$ como la distribuci�n uniforme.
			\begin{itemize}
				\item Entonces, dado que $\vec \pi[a] = \vec \pi[b]$ para todo $a,b$, la condici�n de reversibilidad se reduce a que $P[a,b]=P[b,a]$
			\end{itemize}
			
			\vs{8}
			
			\visible<2->{
				Por lo tanto, no siempre ser� necesario recurrir al algoritmo de Metropolis-Hastings en este contexto.
				\begin{itemize}
					\item Nos basta con definir una cadena de Markov cuya matriz de transici�n sea sim�trica, y que adem�s sea irreducible y aperi�dica
	...
	\begin{frame}
		...
		{\footnotesize
			...
			\begin{proposition}
				Sea $\Omega$ un conjunto finito, $\vec \pi$ una distribuci�n sobre $\Omega$ y $P$ la matriz de transici�n de una cadena de Markov con conjunto de estados $\Omega$. Adem�s, sea $Q$ una matriz tal que:
				\begin{eqnarray*}
					Q[a,b] & = &  
					\begin{cases}
						P[a,b] \cdot \alpha[a,b] + r(b) & \text{si } a = b\\
				...
				\begin{eqnarray*}
					...
					\end{cases}
				\end{eqnarray*}
				Entonces $Q$ es la matriz de transici�n de una cadena de Markov y $Q[d,c] \cdot \vec \pi[c] = Q[c,d] \cdot \vec \pi[d]$ para todo $c,d\in\Omega$
			\end{proposition}
	...
	\begin{frame}
		...
		{\small
			...
			\begin{ejercicio}
				Demuestre la proposici�n anterior.
			\end{ejercicio}
	...
	\begin{frame}
		...
		{\small
			...
			Ahora, solo falta demostrar que el algoritmo de Metropolis-Hastings efect�a las transiciones de acuerdo a las probabilidades especificadas por la matriz $Q$
			
			\vs{10}
			
			\visible<2->{
				En el siguiente teorema, consideramos toda la notaci�n dada en la proposici�n anterior y en la descripci�n del algoritmo de Metropolis-Hastings.}
	...
	\begin{frame}
		...
		{\small
			...
			\visible<2->{
				Demostraremos el teorema sin requerir que $\vec x_0$ sea un vector can�nico.
				\begin{itemize}
					\item $\vec x_0$ representa entonces la distribuci�n de probabilidades de acuerdo a la cual se eligi� $a_0$
			\end{itemize}}
	...
	%{\footnotesize
	%
	%Haremos la demostraci�n por inducci�n, partiendo por el caso base $j=0$
	%\begin{itemize}
	%\item La demostraci�n del caso base es trivial, dado que $Q^0 = I$ y, por hip�tesis, $a_0$ se elige de acuerdo a la distribuci�n de probabilidades representada por $x_0$, o sea, para todo $c\in\Omega$ se tiene
	%    \begin{equation*}
	...
	%\vs{4}
	%
	%Denotamos el primer t�rmino como $p_1$ y el segundo como $p_2$
	%\begin{itemize}
	%\item Estudiamos a continuaci�n cada uno de estos t�rminos
	%\end{itemize}
	%
	%}
	%
	...
	\begin{frame}
		...
		{\footnotesize
			...
			\visible<2->{
				La demostraci�n del caso base es trivial, dado que $Q^0 = I$ y, por hip�tesis, $a_0$ se elige de acuerdo a la distribuci�n de probabilidades representada por $\vec x_0$, o sea, para todo $c\in\Omega$ se tiene:
				\begin{equation*}
					\pr(a_0 = c) \ = \ \vec x_0[c] \ = \ (Q^0 \vec x_0)[c]
				\end{equation*}
			}
	...
	\begin{frame}
		...
		{\small
			...
			\visible<2->{
				As��, queda demostrado el teorema. \qed
			}
	...
	%--------------------------------------------------
	\begin{frame}
		\frametitle{$\vec \pi$ como la �nica distribuci�n estacionaria del algoritmo de Metropolis-Hastings}
		
		{\small
			
			�C�mo podemos asegurar que $\vec \pi$ es la �nica distribuci�n estacionaria de la cadena de Markov con matriz de transici�n $Q$?
			\begin{itemize}
				\item �C�mo podemos asegurar que la cadena de Markov converge a $\vec \pi$ independientemente del vector $\vec x_0$ inicial?
			\end{itemize}
			
			\vs{8}
			
			\visible<2->{
				Para tener las condiciones anteriores la cadena de Markov debe ser irreducible y aperi�dica.}
			\begin{itemize}
				\visible<3->{\item �Para lograr esto basta con que la cadena de Markov con matriz de transici�n $P$ sea irreducible y aperi�dica?}
			\end{itemize}
	...
	%--------------------------------------------------
	\begin{frame}
		\frametitle{No basta partir de una cadena irreducible y aperi�dica}
		...
		{\footnotesize
			...
			Esta cadena de Markov es irreducible y aperi�dica, y su �nica distribuci�n estacionaria es: 
			$$\begin{pmatrix}
			0\text{.}5\\
			0\text{.}25\\
			0\text{.}25
	...
	\begin{frame}
		...
		{\footnotesize
			...
			Queremos utilizar el algoritmo de Metropolis-Hastings para obtener a partir de $P$ la distribuci�n uniforme $\vec \pi[a] = \vec \pi[b] = \vec \pi[c] = \frac{1}{3}$ como distribuci�n estacionaria de una cadena de Markov.
	...
	\begin{frame}
		...
		{\footnotesize
			...
			La cadena de Markov generada por el algoritmo de Metropolis-Hastings no es irreducible ni tiene una �nica distribuci�n estacionaria.
			\begin{itemize}
				\item Y adem�s converge a distintas distribuciones dependiendo de la distribuci�n de partida
			\end{itemize}
			
			\vs{8}
			
			\visible<2->{
				�C�mo podemos solucionar estos problemas?}
	...
	%--------------------------------------------------
	\begin{frame}
		\frametitle{Una soluci�n simple y general}
		
		
		{\footnotesize
			
			Suponga que tenemos una cadena de Markov con conjunto de estados $\Omega$ y matriz de transici�n $P$
			...
			\visible<2->{
				...
				\begin{itemize}
					\item Vale decir, dado $a \in \Omega$, suponemos que es posible generar de manera aleatoria $b\in\Omega$ de acuerdo a la distribuci�n probabilidad $P[\cdot,a]$
					
					\item M�s aun, suponemos que $b$ puede ser generado de manera eficiente 
				\end{itemize}
			}
	...
	\begin{frame}
		...
		{\small
			...
			\begin{eqnarray*}
				U[a,b] & = & \frac{1}{|\Omega|}
			\end{eqnarray*}
			Vale decir, $U$ representa una cadena de Markov en la cual es posible moverse de un estado a cualquier otro con distribuci�n uniforme.
	...
	\begin{frame}
		...
		{\footnotesize
			...
			\begin{itemize}
				...
				\item La matriz $Q_\theta$ generada por el algoritmo de Metropolis-Hastings es irreducible y aperi�dica
				\begin{itemize}
	...
	\begin{frame}
		...
		{\small
			...
			�C�mo podemos asegurar que una cadena de Markov converger� a la distribuci�n estacionaria en un tiempo razonable?
			
			\vs{10}
			
			\visible<2->{
				Para responder esta pregunta, primero necesitamos definir una m�trica que indique la distancia que hay entre la distribuci�n estacionaria y la distribuci�n de probabilidad de la cadena en un tiempo $t \geq 0$
			}
	...
	%--------------------------------------------------
	\begin{frame}
		\frametitle{Distancia de variaci�n}
		
		{\small
			
			Sea $\{ X_t \}_{t \in \mathbb{N}}$ una cadena de Markov con conjunto de estados $\Omega$, matriz de transici�n $P$ y distribuci�n estacionaria $\vec \pi$
			
			\vs{8}
			
			\visible<2->{
				\begin{definicion}
					Dado $x \in \Omega$, la distancia de variaci�n en tiempo $t \geq 0$ con respecto a un estado inicial $x$ se define como:
					\begin{eqnarray*}
	...
	\begin{frame}
		...
		{\small
			...
			\visible<2->{
				...
				�C�mo podemos acotar el mixing time de una cadena de Markov?
	...
	%--------------------------------------------------
	\begin{frame}
		\frametitle{La t�cnica de los caminos can�nicos}
		
		{\small
			
			Desde ahora en adelante, asumiremos una notaci�n est�ndar para los elementos de una cadena de Markov $\{ X_t \}_{t \in \mathbb{N}}$:
			\begin{itemize}
				{\footnotesize
					\item $\Omega$ es su conjunto de estados
					\item $P$ es su matriz de transici�n
					\item $\vec \pi$ es su distribuci�n estacionaria}
			\end{itemize}
	...
	\begin{frame}
		...
		{\footnotesize
			...
			Sea $\{ X_t \}_{t \in \mathbb{N}}$ una cadena de Markov reversible, irreducible y aperi�dica
			...
			\visible<3->{
				Adem�s, dados $x,y \in \Omega$, un camino can�nico de $x$ a $y$ se define como un camino $\gamma_{x,y} = e_1, \ldots, e_r$ desde $x$ a $y$ en $H_{P,\vec \pi}$
				\begin{itemize}
					\visible<4->{
						\item Suponemos que $r \geq 1$, y denotamos al largo $r$ del camino como $|\gamma_{x,y}|$
						\item Utilizamos la notaci�n $e \in \gamma_{x,y}$ para indicar que $e$ es un arco (no dirigido) en el camino $\gamma_{x,y}$}
			\end{itemize}}    
	...
	\begin{frame}
		...
		{\small
			...
			Suponga que por cada par $x,y \in \Omega$ escogemos un camino can�nico $\gamma_{x,y}$, y definimos $\Gamma$ como el conjunto de estos caminos.
			%Sea $\{ X_t \}_{t \in \mathbb{N}}$ una cadena de Markov reversible, irreducible y aperi\'odica, con  grafo subyacente $H=(\Omega, E)$. Adem\'as, sea $\Gamma = \{\gamma_{xy} : x, y \in \Omega\}$ un conjunto de caminos can\'onicos para cada par de estados
			...
			\visible<2->{   
				\begin{definicion}
					La m�xima carga de un arco se define como
					\begin{eqnarray*}
	...
	%--------------------------------------------------
	\begin{frame}
		\frametitle{La t�cnica de los caminos can�nicos: un resultado fundamental}
		
		{\small
			
			\begin{teorema}
				Sea $\{ X_t \}_{t \in \mathbb{N}}$ una cadena de Markov reversible, irreducible y aperi\'odica, tal que $P[x, x] \geq \frac{1}{2}$ para todo $x \in \Omega$. Adem\'as, sea $\Gamma$ un conjunto de caminos can�nicos para cada par de estados en $\Omega$.
...
%--------------------------------------------------
\begin{frame}
	\frametitle{Un recordatorio de �lgebra Lineal}
	
	{\small
		
		Estudiar las cadenas de Markov a trav�s de sus matrices de transici�n nos puede ayudar a enteneder mejor su comportamiento. \pause \visible<2->{Para esto, vamos a recordar unos conceptos de �lgebra Lineal.}
		
		\vs{8}
		
		\visible<3->{
			\begin{definicion}
				Sea $M$ una matriz. Un {\bf vector propio} de $M$ es un vector (no nulo) $\bar{x}$ tal que $M\bar{x} = \lambda\bar{x}$, donde $\lambda\in\R$. Este $\lambda$ a su vez es un {\bf valor propio} de $M$.
			\end{definicion}
		}
		...
		\visible<4->{
			Naturalmente, si $\vec x$ es vector propio de $M$, para cualquier escalar $a\in\R$ se tiene que $a\cdot\vec x$ tambi�n lo es.
		}
	}
...
\begin{frame}
	...
	{\small
		...
		Una cadena de Markov con matriz de transici�n $P$ y una distribuci�n estacionaria $\vec\pi$ va a tener un valor propio 1 y vector propio $\vec\pi$ ya que $P\vec\pi = \pi$.
		
		\vs{4}
		\visible<2->{
			Sin embargo, sabemos bastante m�s de estos valores propios.
		}
		...
		\visible<3->{
			\begin{teorema}
				Considere una cadena de Markov arbitraria con una matriz de transici�n $P$. Se tiene que $1$ es valor propio de $P$ y todos los valores propios de $P$ tienen valor absoluto menor o igual a $1$.
			\end{teorema}
		}
	}
...
\begin{frame}
	...
	{\small
		...
		\vs{4}
		\visible<2->{
			Primero notamos que $\lambda\vec x$ tambi�n es vector propio de $P$ con valor propio $\lambda$, as� que tenemos que $P\lambda \vec x = \lambda^2 \vec x$, luego $P^2\vec x  = \lambda^2\vec x$.
		}
		
		\vs{4}
		\visible<3->{
		 	Extendemos esta idea para un $n$ arbitrario y tenemos que $P^n\vec x = \lambda^n\vec x$, lo que crece exponencialmente cuando $n\to\infty$. 
		}
	
		\vs{4}
		\visible<4->{
			Esto no es posible ya que todas las filas de $P^n$ suman 1. Luego, el valor m�s grande que puede tener una entrada en el vector $\vert \lambda\vert^n\vec x$ es 1.\qed	
		}
	}
...
\end{frame}
	...
	%--------------------------------------------------
	\begin{frame}
		\frametitle{Un primer ejemplo: generaci�n aleatoria de strings binarios}
		
		{\small
			Consideremos el problema de generar con distribuci�n uniforme strings binarios de largo $n \geq 1$ a trav�s de una cadena de Markov.
			\begin{itemize}
			...
			\visible<3->{Para movernos entre estados invertiremos a lo m�s un bit del string.}
			\begin{itemize}
				\visible<4->{\item Con probabilidad $\frac{1}{2}$ no nos movemos de estado}
				\visible<5->{\item Si nos movemos, se invierte uno de los $n$ bits con distribuci�n uniforme}
			\end{itemize}
		}
	...
	\begin{frame}
		...
		{\small
			Sea $\vec \pi : \Omega \rightarrow \mathbb{R}$ la distribuci�n uniforme sobre los estados.
			...
			\visible<3->{
				De esto se concluye que $\vec \pi$ es la (�nica) distribuci�n estacionaria de la cadena de~Markov.
				\begin{itemize}
					\item �Por qu�?
			\end{itemize}}
	...
	\begin{frame}
		...
		{\small
			...
			\begin{itemize}
				\item La cadena es reversible, irreducible, aperi�dica y $P[x,x] \geq \frac{1}{2}$ para cada $x \in \Omega$
			\end{itemize}
			
			\vs{8}
			
			\visible<2->{
				Podemos usar entonces la t�cnica de los caminos can�nicos para acotar el mixing time.}
			\begin{itemize}
				\visible<3->{\item La clave es escoger un conjunto $\Gamma$ de caminos can�nicos tal que $\rho(\Gamma)$ sea polinomial en $n$, que en este caso es el tama�o de la entrada}
			\end{itemize}
			...
			\visible<4->{
				Utilizamos como primer ejemplo a la generaci�n aleatoria de strings binarios porque es simple escoger un conjunto de caminos $\Gamma$ tal que $\rho(\Gamma)$ es polinomial en $n$}
	...
	\begin{frame}
		...
		{\small
			...
			Dado $x = x_1 \cdots x_n$ e $y = y_1 \cdots y_n$, consideramos el camino can�nico $\gamma_{x,y} = e_1 \cdots e_n$, donde para $i \in \{1, \ldots, n\}$:
			\begin{eqnarray*}
			...
			\begin{itemize}
				\visible<3->{\item Si los strings tienen un bit en com�n, utilizamos un loop}
				\visible<4->{\item N�tese que $|\gamma_{xy}| = n$}
			\end{itemize}
		}
	...
	%--------------------------------------------------
	\begin{frame}
		\frametitle{Acotando el n�mero de caminos}
		...
		{\small
			...
			\visible<2->{
				Para alg�n $i \in \{1, \ldots, n\}$, tenemos que:
				\begin{eqnarray*}
	...
	\begin{frame}
		...
		{\small
			...
			\begin{itemize}
				\visible<2->{\item Los bits $x_i$, $\ldots$, $x_n$ de $x$ est�n fijos, y tenemos $2^{i-1}$ opciones para los otros bits de $x$}
				\visible<3->{\item Los bits $y_1$, $\ldots$, $y_i$ de $y$ est�n fijos, y tenemos $2^{n-i}$ opciones para los otros bits de $y$}
			\end{itemize}
	...
	%--------------------------------------------------
	\begin{frame}
		\frametitle{Acotando el mixing time en la generaci�n de strings binarios}
		...
		{\footnotesize
			...
			\visible<3->{
				Concluimos que la cadena converge r�pido desde cualquier estado inicial.
			}
	...
	\begin{frame}
		...
		{\small
			...
			\begin{ejercicio}
				Utilizando el resultado anterior construya un FPAUG para el problema de generar string binarios con distribuci�n uniforme.
				\vs{1}
				\begin{itemize}
					{\footnotesize
						\item Su algoritmo debe realizar a lo m�s la siguiente cantidad de pasos en la cadena de Markov usada: $${\displaystyle \bigg\lceil n^2 \cdot \bigg(n \cdot \ln 2 + \ln \frac{2}{\varepsilon}\bigg)\bigg\rceil}$$}
				\end{itemize}
				\vs{-3}
			\end{ejercicio}
	...
	%--------------------------------------------------
	\begin{frame}
		\frametitle{Permutaciones de una lista: distribuci�n estacionaria}
		
		{\footnotesize
			
			La matriz de transici�n $P$ de la cadena de Markov $\{ X_t \}_{t \in \mathbb{N}}$ satisface lo siguiente para cada $L_1, L_2 \in \Omega$:
			\begin{eqnarray*}
			...
			\visible<2->{
				\begin{ejercicio}
					Sea $\vec \pi : \Omega \rightarrow \mathbb{R}$ la distribuci�n uniforme sobre $\Omega$:
					\begin{eqnarray*}
						\vec \pi[L] & = & \frac{1}{n!} \quad\quad \text{para cada } L \in \Omega
					\end{eqnarray*}
					Demuestre que $\vec \pi$ es la �nica distribuci�n estacionaria de $\{ X_t \}_{t \in \mathbb{N}}$, a la cual converge la cadena independientemente del estado inicial.
			\end{ejercicio}}
	...
	%--------------------------------------------------
	\begin{frame}
		\frametitle{Permutaciones de una lista: caminos can�nicos}
		
		{\small
			Dados $L_1, L_2 \in \Omega$, definimos un camino can�nico $\gamma_{L_1,L_2}$ desde $L_1$ a $L_2$ de la siguiente forma.
			\visible<2->{
				\begin{itemize}
					\item Suponiendo que $L_1 = [a_1, \ldots, a_n]$ y $L_2 = [b_1, \ldots, b_n]$, los arcos de $\gamma_{L_1,L_2}$ van colocando a cada elemento $b_i$ en la posici�n $i$ que le corresponde en $L_2$, desde $i = 1$ hasta $i =n$
					
					\item Si un elemento $b_i$ estaba en la posici�n $i$ en su turno, entonces no se realiza ninguna acci�n
					%    $i$-�simo arco 
					%    de $\gamma_{L_1,L_2}$ corresponde a poner al elemento $b_i$ en la posici�n $i$, si no estaba ah�. 
			\end{itemize}}
			%    \begin{itemize}
			%    \item Si $b_i$ se encontraba en la posici�n $i$ al momento de realizar la transici�n entonces no se cambia la lista (lo que corresponde a utilizar un loop en el grafo $H_{P,\vec \pi}$)
			%    \end{itemize}}
	...
	\begin{frame}
		...
		{\footnotesize
			...
			\begin{ejemplo}
				Si $L_1 = [3,1,4,2]$ y $L_2 = [4,2,1,3]$, el camino can�nico de $L_1$ a $L_2$ se construye realizando las siguientes acciones:
				\begin{enumerate}
					\item Colocar el elemento $4$ en la posici�n $1$, lo que equivale a permutar las posiciones $1$ y $3$ en la lista $L_1$ generando 
					$L_3 = [4,1,3,2]$
					
					\item Colocar el elemento $2$ en la posici�n $2$, lo que equivale a permutar las posiciones $2$ y $4$ en la lista $L_3$ generando 
					$L_4 = [4,2,3,1]$
					
					\item Colocar el elemento $1$ en la posici�n $3$, lo que equivale a permutar las posiciones $3$ y $4$ en la lista $L_4$ generando 
					$L_2 = [4,2,1,3]$
				\end{enumerate}
	...
	\begin{frame}
		...
		{\footnotesize
			...
			\begin{itemize}
				\visible<2->{\item Vamos a acotar superiormente este n�mero}
			\end{itemize}
			...
			\visible<4->{Vamos a definir una funci�n inyectiva $f_{\{L_1,L_2\}} : \{ \gamma_{L_3,L_4} \mid \{L_1,L_2\} \in \gamma_{L_3,L_4}\} \to \Omega$}
			\begin{itemize}
	...
	\begin{frame}
		...
		{\footnotesize
			...
			\visible<2->{
				Nos referimos a la posici�n de un elemento en las permutaciones $L_3$, $L_4$ y $L_1$ como \textit{inicial}, \textit{final} y \textit{actual}, respectivamente.
			}
	...
	\begin{frame}
		...
		{\small
			...
			\begin{itemize}
				\item N�tese que $\gamma_{L_3,L_4}$ est� determinado por $L_3$ y $L_4$
			\end{itemize}
	...
	\begin{frame}
		...
		{\small
			...
			\begin{itemize}
				...
				\item Las transiciones realizadas antes de $\{L_1,L_2\}$ en el camino $\gamma_{L_3,L_4}$ corresponden a poner a un elemento de $a_1, \dotsc, a_{k-1}$ en su posici�n~final
				\vs{1}
	...
	\begin{frame}
		...
		{\small
			...
			\begin{ejercicio}
				...
				\begin{itemize}
					{\footnotesize
						\item Su algoritmo debe realizar a lo m�s la siguiente cantidad de pasos en la cadena de Markov usada: $${\displaystyle \bigg\lceil n^2 \cdot (n-1) \cdot \bigg(n \cdot \ln n + \ln \frac{2}{\varepsilon}\bigg)\bigg\rceil}$$}
				\end{itemize}
				\vs{-3}
			\end{ejercicio}
	...
	%--------------------------------------------------
	\begin{frame}
		\frametitle{Contando el n�mero de matchings de un grafo}
		
		{\small
			Dado un grafo $G$ no dirigido, sea $\smatching(G)$ una funci�n que retorna el n�mero de matchings de $G$
			...
			\visible<3->{
				...
				\begin{itemize}
					\item �Qu� herramientas podemos usar para construir este FPRAS?
				\end{itemize}
			}
	...
	\begin{frame}
		...
		{\footnotesize
			...
			Considere la siguiente relaci�n:
			\begin{multline*}
			...
			\visible<2->{
				\begin{proposition}
					...
					\begin{itemize}
						\item �C�mo puede ser codificado $M$ para que se cumpla esta propiedad?
					\end{itemize}
			\end{proposition}}
	...
	\begin{frame}
		...
		{\footnotesize
			...
			Dado $M \in \Omega$, el procedimiento para ejecutar una transici�n es el siguiente:
			\begin{enumerate}
				\visible<2->{\item Con probabilidad $\frac{1}{2}$ nos quedamos en $M$}
				\visible<3->{\item En caso contrario, escogemos un arco $e \in M$, con $e = \{u, v\}$ y $u \neq v$, de manera aleatoria y con distribuci�n uniforme, y hacemos lo siguiente:}
				\begin{enumerate}
					{\footnotesize 
						...
						\visible<6->{\item si $|e \cap e'| = 1$ para alg�n $e' \in E$, y $e \cap e'' = \emptyset$ para todo $e'' \in M \smallsetminus \{e'\}$, entonces nos movemos a $(M \smallsetminus \{e'\}) \cup \{e\}$}
	...
	\begin{frame}
		...
		{\small
			...
			\begin{exampleblock}{Ejercicios}
				...
				\vs{1}
				\begin{enumerate}
					\item Demuestre que la cadena es irreducible y aperi�dica
					\vs{1}
					\item Demuestre que la cadena es reversible y su distribuci�n estacionaria es la distribuci�n uniforme sobre $\Omega$
				\end{enumerate}
			\end{exampleblock}
	...
	%--------------------------------------------------
	\begin{frame}
		\frametitle{Matchings de un grafo: caminos can�nicos}
		...
		{\small
			...
			Suponemos dado un orden lineal para el conjunto de los caminos y ciclos simples en $G$, y suponemos que cada uno de ellos tiene un v�rtice inicial.
			\begin{itemize}
				\visible<2->{\item Si el camino no es un ciclo, el v�rtice inicial tiene que ser un extremo
					\item Si el camino es un ciclo, el v�rtice inicial es un nodo arbitrario}
			\end{itemize}
		}
	\end{frame}
	...
	\begin{frame}
		...
		{\small
			...
			\only<2|handout:2>{
				Y un v�rtice inicial para cada camino:
				\begin{columns}
					\begin{column}{0.5\textwidth}
						\begin{itemize}
							\item $Q_1 = (\alert{1}, 2)$
	...
	%--------------------------------------------------
	\begin{frame}
		\frametitle{Definici�n de los caminos can�nicos}
		...
		{\small
			...
			\visible<2->{
				Podemos describir la diferencia sim�trica $(I \smallsetminus F) \cup (F \smallsetminus I)$ como una secuencia $Q_1, \ldots, Q_r$ de caminos y ciclos simples disjuntos que respetan el orden lineal.
				\begin{itemize}
					\item Construimos el camino can�nico de $I$ a $F$ secuencialmente a partir de estos caminos y ciclos simples
					\begin{itemize}
						{\footnotesize
	...
	\begin{frame}
		...
		{\footnotesize
			...
			Sea $Q_i = (v_0, v_1, \dotsc, v_\ell)$, con $v_i \in \Omega$ para todo $i \in \{0, \ldots, \ell\}$ y $v_0$ el v�rtice~inicial
			...
			\begin{itemize}
				...
				\visible<5->{\item Finalizamos con una transici�n de tipo 2 si $\ell$ es impar}
				\visible<6->{
					\begin{itemize}
						{\footnotesize
							\alert{\item Si $\ell$ es impar, falta considerar el arco $\{v_{\ell-1}, v_\ell\}$, que agregamos a trav�s de una transici�n de tipo 2}}
					\end{itemize}
				}
			\end{itemize}
		}
	...
	\begin{frame}
		...
		{\footnotesize
			...
			\vs{1}
			\begin{itemize}
				\visible<3->{\item Partimos con una transici�n de tipo 1, removiendo $\{v_0, v_1\}$}
				\visible<4->{\item Procedemos con el camino reducido $(v_1, \dotsc, v_\ell)$}
				\visible<5->{
					\begin{itemize}
						{\footnotesize
							\alert{\item N�tese que si el camino $(v_1, \dotsc, v_\ell)$ tiene a $v_\ell$ como v�rtice inicial y $\{v_{\ell-1}, v_\ell\} \in I$ entraremos nuevamente en este caso}}
					\end{itemize}
				}
			\end{itemize}
		}
	...
	\begin{frame}
		...
		{\footnotesize
			...
			Sea $Q_i = (v_0, v_1, \dotsc, v_{2\ell+1}, v_0)$, con $v_i \in \Omega$ para todo $i \in \{0 , \ldots, 2\ell+1\}$, $v_0$ el v�rtice inicial y $\{v_{2j}, v_{2j+1}\} \in I$ para todo $j \in \{0, \ldots, \ell\}$
			\vs{1}
			\begin{itemize}
				\visible<2->{\item El ciclo debe tener un n�mero par de aristas. �Por qu�?}
				\visible<3->{\item N�tese que el orden del ciclo no es relevante, por lo que si $\{v_{2j}, v_{2j+1}\} \in F$ para todo $j \in \{0, \ldots, \ell\}$, podemos considerar el ciclo en el orden invertido y cumplir� con las condiciones pedidas}
			\end{itemize}
	...
	\begin{frame}
		...
		{\footnotesize
			...
			\vs{1}
			\begin{itemize}
				\visible<2->{\item Partimos com una transici�n de tipo 1 removiendo $\{v_0, v_1\}$}
				\visible<3->{\item Denotamos al camino simple resultante como $O = (v_1, \dotsc, v_{2j+1}, v_0)$, con extremos $v_1, v_0$}
				\visible<4->{\item Tratamos a $O$ igual que cualquier camino simple utilizando $v_0$ o $v_1$ como el v�rtice inicial}
				\visible<5->{
					\begin{itemize}
						{\footnotesize
							\alert{\item Se utiliza el orden inverso para $O$ para diferenciar a los ciclos simples de los caminos simples durante el an�lisis del mixing time de la cadena de Markov}}
					\end{itemize}
				}
			\end{itemize}
		}
	...
	%--------------------------------------------------
	\begin{frame}
		\frametitle{Definici�n de los caminos can�nicos: un ejemplo}
		
		
		{\small
			
			Consideremos el ejemplo introducido anteriormente. Supongamos que queremos encontrar el camino can�nico entre los siguientes matchings:
	...
	\begin{frame}
		...
		{\small
			...
			La diferencia sim�trica \alert{$(I \smallsetminus F) \cup (F \smallsetminus I) = \{\{1, 3\}, \{2, 4\}, \{1, 2\}\}$}, lo que corresponde al camino simple $Q_{11} = (\alert{3}, 1, 2, 4)$
			\begin{itemize}
			...
			\visible<3->{Luego, el camino can�nico entre estos estados corresponde a:}
			\begin{itemize}
				\visible<4->{\item Utilizar una transici�n de tipo 1 para remover $\{3, 1\}$}
				\visible<5->{\item Utilizar una transici�n de tipo 3 para remplazar $\{2, 4\}$ por $\{1, 2\}$}
			\end{itemize}
	...
	\begin{frame}
		...
		{\small
			...
			En un ejemplo m�s extenso, simplemente realizamos las transiciones correspondientes a cada camino y ciclo simple en el orden establecido entre ellos.
			\begin{itemize}
				\visible<2->{\item N�tese que, dado que los caminos y ciclos simples utilizados tienen~que ser disjuntos, solo habr� una forma de crear cada camino~can�nico}
			\end{itemize}
	...
	\begin{frame}
		...
		{\small
			A partir del conjunto de caminos can�nicos descritos, podemos establecer la siguiente cota para el mixing time de la cadena.
